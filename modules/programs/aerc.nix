...
let
  ...
  maildirSource = a:
    ...
    let proto = if a.notmuch.enable then "notmuch" else "maildir";
    in "source = ${proto}://${a.maildirBasePath}/${a.maildir.path}";

  outgoing = a:
    if a.aerc.sendMailCommand != null then
      "outgoing = ${a.aerc.sendMailCommand}"
    else
      assert a.smtp != null;
      with a.smtp;
      let
  ...
  accountSection = a: ''
    ...
    ${optionalString (a.signature != "") "signature-file = ${signature a}"}
...
in {
  ...
  config = mkIf cfg.enable {
    home.packages = [ pkgs.aerc.override {
        patches = [
    ...
    xdg.configFile."aerc/aerc.conf" = mkIf (aercAccounts != [ ]) {
      text = ''
        # Generated by Home Manager.
    ...
    xdg.configFile."aerc/binds.conf" = mkIf (aercAccounts != [ ]) {
      text = ''
        # Generated by Home Manager.

        ${genBinds cfg.bindContexts.globalBinds}
        ${mapAttrs genBindContextSection
        (removeAttrs cfg.bindContexts [ "globalBinds" ])}
      '';
    };
  };
}
